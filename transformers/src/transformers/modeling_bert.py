...
class RelationNegotiator(nn.Module):
    """Lightweight subject-object interaction layer that supports multiple dialogue rounds."""

    def __init__(self, hidden_dim, dropout_prob, num_rounds=1, eps=1e-6):
        super().__init__()
        self.subject_to_object = nn.Sequential(
            nn.Linear(hidden_dim * 2, hidden_dim),
            nn.GELU(),
            nn.Linear(hidden_dim, hidden_dim),
        )
        self.object_to_subject = nn.Sequential(
            nn.Linear(hidden_dim * 2, hidden_dim),
            nn.GELU(),
            nn.Linear(hidden_dim, hidden_dim),
        )
        self.feedback_refiner = nn.Sequential(
            nn.Linear(hidden_dim * 2, hidden_dim),
            nn.GELU(),
            nn.Linear(hidden_dim, hidden_dim),
        )
        ...
        self.object_refine_norm = nn.LayerNorm(hidden_dim)
        self.eps = eps
        self.num_rounds = num_rounds

        # Gating layers
        self.gate_s2o = nn.Linear(hidden_dim * 2, hidden_dim)
        self.gate_o2s = nn.Linear(hidden_dim * 2, hidden_dim)
        self.gate_refine = nn.Linear(hidden_dim * 2, hidden_dim)

    def forward(self, subject_states, object_states, pair_mask, num_rounds_override=None):
        if pair_mask is None:
            pair_mask = object_states.new_ones(object_states.size(0), object_states.size(1))

        mask = pair_mask.to(object_states.dtype).unsqueeze(-1)

        num_rounds = self.num_rounds if num_rounds_override is None else int(num_rounds_override)
        if num_rounds <= 0:
            return subject_states, object_states

        for _ in range(num_rounds):
            subject_expanded = subject_states.unsqueeze(1).expand_as(object_states)
            object_input = torch.cat([object_states, subject_expanded], dim=-1)
            object_delta = self.subject_to_object(object_input) * mask
            
            # Gated update for object_states
            # breakpoint()
            gate_s2o_value = torch.sigmoid(self.gate_s2o(torch.cat([object_states, object_delta], dim=-1)))
            object_states = self.object_norm(object_states + self.dropout(gate_s2o_value * object_delta))
            ...
            pm_bool = pair_mask.to(dtype=torch.bool)
            attn_logits = attn_logits.masked_fill(~pm_bool, -1e4)
            attn = torch.softmax(attn_logits, dim=-1)
            aggregated_attn = torch.sum(object_states * attn.unsqueeze(-1), dim=1)
            denom = mask.sum(dim=1).clamp_min(self.eps)
            aggregated_mean = torch.sum(object_states * mask, dim=1) / denom
            ...
            subject_input = torch.cat([subject_states, aggregated], dim=-1)
            subject_delta = self.object_to_subject(subject_input)

            # Gated update for subject_states
...
class BertForACEBothOneDropoutSub(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.max_seq_length = config.max_seq_length
        self.num_labels = config.num_labels
        self.num_dialogue_rounds = getattr(config, "num_dialogue_rounds", 1)
        self.num_ner_labels = config.num_ner_labels
        self.max_pair_length = config.max_pair_length
        ...
        self.alpha = torch.tensor([config.alpha] + [1.0] * (self.num_labels-1), dtype=torch.float32)
        self.candidate_top_n = getattr(config, "candidate_top_n", 0)
        self.candidate_worst_m = getattr(config, "candidate_worst_m", 0)
        self.max_candidate_length = getattr(config, "max_candidate_length", 0)
        self.candidate_adaptive = getattr(config, "candidate_adaptive", False)
        # Adaptive Top+Bottom controls
        self.candidate_adaptive_include_bottom = getattr(config, "candidate_adaptive_include_bottom", False)
        self.candidate_adaptive_top_max = getattr(config, "candidate_adaptive_top_max", 4)
        self.candidate_adaptive_bottom_max = getattr(config, "candidate_adaptive_bottom_max", 4)
        # Dynamic gating controls for adaptive mode
        self.candidate_adaptive_dynamic_top = getattr(config, "candidate_adaptive_dynamic_top", False)
        self.candidate_adaptive_top_min = getattr(config, "candidate_adaptive_top_min", 1)
        self.candidate_adaptive_dynamic_bottom = getattr(config, "candidate_adaptive_dynamic_bottom", False)
        self.candidate_adaptive_bottom_min = getattr(config, "candidate_adaptive_bottom_min", 0)
        self.candidate_adaptive_confidence_metric = getattr(config, "candidate_adaptive_confidence_metric", "entropy")
        self.enable_candidate = False
        self._use_candidate_features = self.candidate_adaptive or ((self.candidate_top_n + self.candidate_worst_m) > 0)
        if self._use_candidate_features:
            self.candidate_label_embeddings = nn.Embedding(self.num_labels, config.hidden_size * 2)
        ...
        self.relation_negotiator = RelationNegotiator(
            config.hidden_size * 2,
            config.hidden_dropout_prob,
            num_rounds=self.num_dialogue_rounds,
        )

        self.init_weights()

    def set_candidate_mode(self, enabled: bool):
        self.enable_candidate = bool(enabled)

    def _candidate_enrichment(self, scores, pair_mask=None, labels=None):
        if not (self._use_candidate_features and self.enable_candidate):
            return None

        if pair_mask is None:
            if labels is not None:
                pair_mask = (labels != -1).to(scores.dtype)
            else:
                pair_mask = scores.new_ones(scores.size(0), scores.size(1))
        else:
            pair_mask = pair_mask.to(scores.dtype)

        bsz, ent_len, num_labels = scores.size()

        if self.candidate_adaptive:
            # Adaptive selection with optional Top+Bottom fusion
            probs = torch.softmax(scores, dim=-1)
            entropy = -(probs * (probs.clamp_min(1e-8)).log()).sum(dim=-1)  # [bsz, ent_len]
            inv_entropy = 1.0 / (entropy + 1e-6)

            # Capacity limit
            max_k = self.max_candidate_length if self.max_candidate_length > 0 else num_labels
            max_k = min(max_k, num_labels)

            # Compute confidence in [0,1]
            if self.candidate_adaptive_confidence_metric == "max_prob":
                conf = probs.max(dim=-1).values  # [bsz, ent_len]
            else:
                max_entropy = math.log(num_labels) if num_labels > 1 else 1.0
                conf = 1.0 - (entropy / max_entropy)
            conf = conf.clamp(0.0, 1.0)

            # Determine per-pair Top count (dynamic or fixed)
            top_k_max = min(self.candidate_adaptive_top_max if self.candidate_adaptive_top_max > 0 else max_k, max_k)
            if self.candidate_adaptive_dynamic_top:
                top_min = max(1, int(self.candidate_adaptive_top_min))
                top_min_t = torch.tensor(float(top_min), device=scores.device)
                top_max_t = torch.tensor(float(top_k_max), device=scores.device)
                # High confidence -> fewer Top; Low confidence -> more Top
                top_k_float = top_min_t + (1.0 - conf) * (top_max_t - top_min_t)
                top_k_per_pair = top_k_float.round().to(torch.long)  # [bsz, ent_len]
            else:
                top_k_per_pair = torch.full((bsz, ent_len), top_k_max, device=scores.device, dtype=torch.long)

            # Determine per-pair Bottom cap and counts (dynamic or fixed)
            bottom_cap = self.candidate_adaptive_bottom_max if (self.candidate_adaptive_include_bottom and self.candidate_adaptive_bottom_max > 0) else 0
            if bottom_cap > 0:
                if self.candidate_adaptive_dynamic_bottom:
                    bottom_min = max(0, int(self.candidate_adaptive_bottom_min))
                    bottom_min_t = torch.tensor(float(bottom_min), device=scores.device)
                    bottom_max_t = torch.tensor(float(bottom_cap), device=scores.device)
                    bottom_k_float = bottom_min_t + (1.0 - conf) * (bottom_max_t - bottom_min_t)
                    bottom_k_per_pair = bottom_k_float.round().to(torch.long)
                else:
                    bottom_k_per_pair = torch.full((bsz, ent_len), bottom_cap, device=scores.device, dtype=torch.long)
                # Enforce capacity: bottom_k <= max_k - top_k
                bottom_k_per_pair = torch.minimum(bottom_k_per_pair, (max_k - top_k_per_pair).clamp(min=0))
            else:
                bottom_k_per_pair = None

            segments_indices = []
            segments_values = []

            # Always compute up to max slots, then mask excess by -1e9
            if top_k_max > 0:
                top_values, top_indices = torch.topk(scores, top_k_max, dim=-1)
                # Mask out per-pair extra tops beyond dynamic top_k_per_pair
                top_range = torch.arange(top_k_max, device=scores.device).view(1, 1, -1)
                top_mask = top_range < top_k_per_pair.unsqueeze(-1)  # [bsz, ent_len, top_k_max]
                top_values = torch.where(top_mask, top_values, scores.new_full(top_values.shape, -1e9))
                segments_indices.append(top_indices)
                segments_values.append(top_values)

            if bottom_cap and bottom_cap > 0:
                worst_values, worst_indices = torch.topk(-scores, bottom_cap, dim=-1)
                worst_values = -worst_values
                bottom_range = torch.arange(bottom_cap, device=scores.device).view(1, 1, -1)
                bottom_mask = bottom_range < bottom_k_per_pair.unsqueeze(-1)  # [bsz, ent_len, bottom_cap]
                worst_values = torch.where(bottom_mask, worst_values, scores.new_full(worst_values.shape, -1e9))
                segments_indices.append(worst_indices)
                segments_values.append(worst_values)

            if not segments_indices:
                return None

            candidate_indices = torch.cat(segments_indices, dim=-1)
            candidate_values = torch.cat(segments_values, dim=-1)

            weight = inv_entropy.unsqueeze(-1)
            candidate_values = candidate_values * weight
            mask = pair_mask.unsqueeze(-1)
            candidate_values = candidate_values * mask
            attention = torch.softmax(candidate_values, dim=-1)
            candidate_embeddings = self.candidate_label_embeddings(candidate_indices)
            enriched = torch.sum(candidate_embeddings * attention.unsqueeze(-1), dim=-2)
            enriched = enriched * mask
            return self.candidate_projector(enriched)
        else:
            total_select = self.candidate_top_n + self.candidate_worst_m
            if total_select == 0:
                return None
            ...
            if self.candidate_top_n > 0:
                top_k = min(self.candidate_top_n, num_labels)
                top_values, top_indices = torch.topk(scores, top_k, dim=-1)
                top_segments.append(top_indices)
                value_segments.append(top_values)

            if self.candidate_worst_m > 0:
                worst_k = min(self.candidate_worst_m, num_labels)
                worst_values, worst_indices = torch.topk(-scores, worst_k, dim=-1)
                top_segments.append(worst_indices)
                value_segments.append(-worst_values)

            if not top_segments:
                return None

            candidate_indices = torch.cat(top_segments, dim=-1)
            candidate_values = torch.cat(value_segments, dim=-1)
            mask = pair_mask.unsqueeze(-1)
            candidate_values = candidate_values * mask
    ...
    def forward(
        ...
        seq_len = self.max_seq_length
        bsz, tot_seq_len = input_ids.shape
        ent_len = self.max_pair_length

        e1_hidden_states = hidden_states[:, seq_len:seq_len+ent_len]
        e2_hidden_states = hidden_states[:, seq_len+ent_len:seq_len + ent_len*2 ]
        ...
        if pair_mask is None:
            if labels is not None:
                pair_mask = (labels != -1).to(feature_vector.dtype)
            else:
                pair_mask = feature_vector.new_ones(feature_vector.size(0), feature_vector.size(1))
        else:
            pair_mask = pair_mask.to(feature_vector.dtype)

        m1_states, feature_vector = self.relation_negotiator(
            m1_states, feature_vector, pair_mask, num_rounds_override=dialogue_rounds_override
        )
        ...
        candidate_delta = self._candidate_enrichment(re_prediction_scores, pair_mask=pair_mask, labels=labels)
        if candidate_delta is not None:
            weight = 1.0 if candidate_weight is None else float(candidate_weight)
            feature_vector = feature_vector + candidate_delta * weight
...
class BertForACEBothOneDropoutSubNoNer(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.max_candidate_length = getattr(config, "max_candidate_length", 0)
        self.enable_candidate = False
        self._use_candidate_features = (self.candidate_top_n + self.candidate_worst_m) > 0
        if self._use_candidate_features:
            self.candidate_label_embeddings = nn.Embedding(self.num_labels, config.hidden_size * 2)
            self.candidate_projector = nn.Sequential(
                nn.Linear(config.hidden_size * 2, config.hidden_size * 2),
    ...
    def _candidate_enrichment(self, scores, pair_mask=None, labels=None):
        ...
        if self.candidate_top_n > 0:
            top_k = min(self.candidate_top_n, scores.size(-1))
            top_values, top_indices = torch.topk(scores, top_k, dim=-1)
            top_segments.append(top_indices)
            value_segments.append(top_values)

        if self.candidate_worst_m > 0:
            worst_k = min(self.candidate_worst_m, scores.size(-1))
            worst_values, worst_indices = torch.topk(-scores, worst_k, dim=-1)
            top_segments.append(worst_indices)
            value_segments.append(-worst_values)
