...
task_ner_labels = {
    ...
    # SCINLP: entity types from the paper table (Task, Dataset, Metric, Model)
    'scinlp': ['Task', 'Dataset', 'Metric', 'Model', 'Method', 'Tool'],
}
...
task_rel_labels = {
    ...
    # SCINLP: relation types from the paper table
    'scinlp': ['measuredBy', 'evaluatedBy', 'usedFor', 'enhancedBy', 'evaluatedOn', 'partOf', 'compareWith', 'subtaskOf', 'similarWith', 'trainedWith', 'subclassOf'],
}
...
class ACEDataset(Dataset):
    def __init__(self, tokenizer, args=None, evaluate=False, do_test=False, max_pair_length=None):
        ...
        # Graceful fallback for external test result paths (e.g., HGERE outputs)
        # Some pipelines save test NER predictions as 'pred_ent_pred_test.json'.
        if not os.path.isfile(file_path) and evaluate and do_test and args.test_file.find('models') != -1:
            base_dir = os.path.dirname(file_path)
            alt_file = os.path.join(base_dir, 'pred_ent_pred_test.json')
            if os.path.isfile(alt_file):
                logger.info("Fallback to test file: %s", alt_file)
                file_path = alt_file
        ...
        self.candi_top_n = args.candidate_top_n
        self.candi_worst_m = args.candidate_worst_m
        self.max_candi_length = args.max_candidate_length
        ...
        # SCINLP 数据集：使用论文中的实体与关系类型（大小写敏感，保留原拼写）
        elif args.data_dir.lower().find('scinlp')!=-1:
            # Entities: Task, Dataset, Metric, Model
            self.ner_label_list = ['NIL', 'Task', 'Dataset', 'Metric', 'Model', 'Method', 'Tool']

            # Relations: measuredBy, evaluatedBy, usedFor, enhancedBy, evaluatedOn, partOf,
            #            compareWith, subtaskOf, similarWith, trainedWith, subclassOf
            # 对称关系：compareWith、similarWith；其余关系视为有向
            if args.no_sym:
                label_list = ['measuredBy', 'evaluatedBy', 'usedFor', 'enhancedBy', 'evaluatedOn', 'partOf', 'subtaskOf', 'trainedWith', 'subclassOf', 'compareWith', 'similarWith']
                self.sym_labels = ['NIL']
                self.label_list = self.sym_labels + label_list
            else:
                label_list = ['measuredBy', 'evaluatedBy', 'usedFor', 'enhancedBy', 'evaluatedOn', 'partOf', 'subtaskOf', 'trainedWith', 'subclassOf']
                self.sym_labels = ['NIL', 'compareWith', 'similarWith']
                self.label_list = self.sym_labels + label_list
    ...
    def initialize(self):
        ...
        # Normalize helpers for SCINLP to handle lowercase/snake-case labels from data files
        is_scinlp = self.args.data_dir.lower().find('scinlp') != -1
        if is_scinlp:
            import re as _re
            def _norm_entity(lbl: str) -> str:
                if lbl is None:
                    return lbl
                ...
                return mapping.get(m, lbl)
            def _norm_relation(lbl: str) -> str:
                if lbl is None:
                    return lbl
                k = _re.sub(r'[^a-zA-Z]', '', str(lbl)).lower()
                mapping = {
                    ...
                    'subtaskof': 'subtaskOf',
                    'similarwith': 'similarWith',
                    'trainedwith': 'trainedWith',
                    'subclassof': 'subclassOf',
                }
                return mapping.get(k, lbl)
            # Extend maps to accept lowercase keys directly when encountered
            for k, v in list(ner_label_map.items()):
                ner_label_map[k.lower()] = v
            for k, v in list(label_map.items()):
                label_map[k.lower()] = v
                label_map[_re.sub(r'[^a-zA-Z]', '', k).lower()] = v
        ...
        for l_idx, line in enumerate(f):
            ...
            # Canonicalize SCINLP labels for robustness
            if is_scinlp:
                try:
                    ners = [ [(s, e, _norm_entity(l)) for (s, e, l) in sent] for sent in ners ]
                except Exception:
                    pass
                try:
                    std_ners = [ [(s, e, _norm_entity(l)) for (s, e, l) in sent] for sent in std_ners ]
                except Exception:
                    pass
                try:
                    relations = [ [(a, b, c, d, _norm_relation(l)) for (a, b, c, d, l) in sent] for sent in relations ]
                except Exception:
                    pass
            ...
            for n in range(len(subword_sentence_boundaries) - 1):
                ...
                for x in sentence_relations:
                    if is_scinlp:
                        x = (x[0], x[1], x[2], x[3], _norm_relation(x[4]))
                    if x[4] not in label_map:
                        logger.warning("Unknown relation label encountered: %s; skipping.", x[4])
                        continue
                ...
                for x in sentence_relations:
                    w = (x[2],x[3],x[0],x[1])
                    if w not in pos2label and x[4] in label_map:
                        if x[4] in self.sym_labels[1:]:
                            pos2label[w] = label_map[x[4]]
                        else:
                            # Use the true number of label classes (self.label_list), not dict size
                            pos2label[w] = label_map[x[4]] + len(self.label_list) - len(self.sym_labels)
                ...
                for sub in entities:    
                    ...
                    for start, end, obj_label in sentence_ners:
                        if is_scinlp:
                            obj_label = _norm_entity(obj_label)
                        if obj_label not in ner_label_map:
                            logger.warning("Unknown entity label encountered: %s; skipping.", obj_label)
                            continue
                        ...
                        if not (0 <= label < self.args.num_labels):
                            logger.error("Label index out of range: %s (num_labels=%s) for relation between %s and %s; skipping example.", label, self.args.num_labels, (sub[0], sub[1]), (obj[0], obj[1]))
                            continue
    ...
    def __getitem__(self, idx):
        ...
        if self.args.att_left:
            ...
            attention_mask[self.max_seq_length : self.max_seq_length+pair_L, self.max_seq_length+num_pair : self.max_seq_length+num_pair+pair_L] = 1
        if self.args.att_right:
            ...
            attention_mask[self.max_seq_length+num_pair : self.max_seq_length+num_pair+pair_L, self.max_seq_length : self.max_seq_length+pair_L] = 1

        mention_pos += [(0, 0)] * (num_pair - len(mention_pos))
        labels += [-1] * (num_pair - len(labels))
        ner_labels += [-1] * (num_pair - len(ner_labels))
        pair_mask = [1] * pair_L + [0] * (num_pair - pair_L)
        ...
        item = [torch.tensor(input_ids),
                ...
                torch.tensor(pair_mask, dtype=torch.float32),
...
def _rotate_checkpoints(args, checkpoint_prefix, use_mtime=False):
    # Retain all checkpoints; do not delete older ones.
    return

def _next_checkpoint_index(output_dir, checkpoint_prefix):
    pattern = os.path.join(output_dir, '{}-*'.format(checkpoint_prefix))
    existing = glob.glob(pattern)
    max_idx = 0
    for path in existing:
        m = re.match(r'.*{}-([0-9]+)'.format(checkpoint_prefix), path)
        if m and m.groups():
            try:
                idx = int(m.groups()[0])
                if idx > max_idx:
...
def train(args, model, tokenizer):
    ...
    warming_step = args.st1_warming_up * t_total
    logger.info("Warming-up step threshold: %s", warming_step)
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            attention_mask : torch.Size([8, 288, 288])
            position_ids : torch.Size([8, 288])
            labels : torch.Size([8, 16])
            ner_labels : torch.Size([8, 16])
            sub_positions : torch.Size([8, 2])
            """    

            # 暖启动策略：在暖启动阶段启用候选增强，并在暖启动结束后再开启多轮对话
            is_warm = global_step < warming_step
            model_to_control = model.module if hasattr(model, 'module') else model
            # 候选增强开关仅依据固定 N/M：若 N+M>0 则启用，权重在暖启动线性上升
            enable_train_candidates = ((getattr(args, 'candidate_top_n', 0) + getattr(args, 'candidate_worst_m', 0)) > 0)
            if hasattr(model_to_control, 'set_candidate_mode'):
                model_to_control.set_candidate_mode(enable_train_candidates)
            candidate_weight = 0.0
            if enable_train_candidates:
                if is_warm:
                    denom = max(1.0, warming_step)
                    candidate_weight = max(0.0, min(1.0, global_step / denom))
                else:
                    candidate_weight = 1.0
            inputs['candidate_weight'] = candidate_weight

            # 多轮对话在暖启动阶段关闭（override为0），暖启动后使用配置默认轮次（None）
            if 'bertsub' in args.model_type or 'bertnonersub' in args.model_type:
                inputs['dialogue_rounds_override'] = 0 if is_warm else None
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    # Log metrics
                    lr_val = scheduler.get_lr()[0]
                    avg_loss = (tr_loss - logging_loss)/args.logging_steps
                    avg_re_loss = (tr_re_loss - logging_re_loss)/args.logging_steps
                    avg_ner_loss = (tr_ner_loss - logging_ner_loss)/args.logging_steps
                    # 计算当前global_step下的记录值以保持一致性
                    is_warm_log = global_step < warming_step
                    denom = max(1.0, warming_step)
                    candidate_weight_log = (max(0.0, min(1.0, global_step / denom)) if is_warm_log else 1.0)

                    tb_writer.add_scalar('lr', lr_val, global_step)
                    tb_writer.add_scalar('loss', avg_loss, global_step)
                    tb_writer.add_scalar('RE_loss', avg_re_loss, global_step)
                    tb_writer.add_scalar('NER_loss', avg_ner_loss, global_step)
                    tb_writer.add_scalar('candidate_weight', candidate_weight_log, global_step)
                    tb_writer.add_scalar('enable_candidate', int(enable_train_candidates), global_step)
                    tb_writer.add_scalar('dialogue_rounds_override', 0 if is_warm_log else -1, global_step)
                    ...
                    # 追加CSV记录
                    train_metrics_rows.append([global_step, lr_val, avg_loss, avg_re_loss, avg_ner_loss, candidate_weight_log, 1, 0 if is_warm_log else None])
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0: # valid for bert/spanbert
                    ...
                    if args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                        # 评估时不再根据暖启动动态改写对话轮次，保持配置一致
                        results = evaluate(args, model, tokenizer)
                        f1 = results['f1_with_ner']
                        tb_writer.add_scalar('f1_with_ner', f1, global_step)
                        # Append eval metrics for CSV export
                        eval_metrics_rows.append([
                            global_step,
                            results.get('f1_with_ner', None),
                            results.get('f1', None),
                            results.get('ner_f1', None),
                            results.get('enable_eval_candidates', None)
                        ])
    ...
    model_to_control = model.module if hasattr(model, 'module') else model
    if hasattr(model_to_control, 'set_candidate_mode'):
        model_to_control.set_candidate_mode(False)

    if args.local_rank in [-1, 0]:
        tb_writer.close()

        # Export CSV and PNG plots
        try:
            ...
            with open(train_csv_path, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(["global_step", "lr", "loss", "RE_loss", "NER_loss", "candidate_weight", "enable_candidate", "dialogue_rounds_override"])
                writer.writerows(train_metrics_rows)

            eval_csv_path = os.path.join(args.output_dir, "eval_metrics.csv")
            with open(eval_csv_path, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(["global_step", "f1_with_ner", "f1", "ner_f1", "enable_eval_candidates"])
                writer.writerows(eval_metrics_rows)

            logger.info("Saved training metrics CSV to %s", train_csv_path)
            logger.info("Saved evaluation metrics CSV to %s", eval_csv_path)

            if HAVE_MATPLOTLIB:
                plots_dir = os.path.join(args.output_dir, "plots")
                os.makedirs(plots_dir, exist_ok=True)

                if len(train_metrics_rows) > 0:
                    steps = [row[0] for row in train_metrics_rows]
                    losses = [row[2] for row in train_metrics_rows]
                    re_losses = [row[3] for row in train_metrics_rows]
                    ner_losses = [row[4] for row in train_metrics_rows]
                    plt.figure(figsize=(8, 5))
                    plt.plot(steps, losses, label="loss")
                    plt.plot(steps, re_losses, label="RE_loss")
                    plt.plot(steps, ner_losses, label="NER_loss")
                    plt.xlabel("global_step")
                    plt.ylabel("loss")
                    plt.legend()
                    plt.tight_layout()
                    plt.savefig(os.path.join(plots_dir, "train_losses.png"))
                    plt.close()

                if len(eval_metrics_rows) > 0:
                    steps = [row[0] for row in eval_metrics_rows]
                    f1_with_ner_list = [row[1] for row in eval_metrics_rows]
                    f1_list = [row[2] for row in eval_metrics_rows]
                    ner_f1_list = [row[3] for row in eval_metrics_rows]
                    plt.figure(figsize=(8, 5))
                    plt.plot(steps, f1_with_ner_list, label="f1_with_ner")
                    if any(x is not None for x in f1_list):
                        plt.plot(steps, [x if x is not None else float('nan') for x in f1_list], label="f1")
                    if any(x is not None for x in ner_f1_list):
                        plt.plot(steps, [x if x is not None else float('nan') for x in ner_f1_list], label="ner_f1")
                    plt.xlabel("global_step")
                    plt.ylabel("F1")
                    plt.legend()
                    plt.tight_layout()
                    plt.savefig(os.path.join(plots_dir, "eval_f1.png"))
                    plt.close()

                logger.info("Saved plots to %s", plots_dir)
            else:
                logger.warning("matplotlib not available; skipped PNG generation.")
        except Exception as e:
            logger.warning("Failed to export curves: %s", str(e))
...
def evaluate(args, model, tokenizer, prefix="", do_test=False):
    ...
    model_to_control = model.module if hasattr(model, 'module') else model
    prev_candidate_enabled = None
    if hasattr(model_to_control, 'set_candidate_mode'):
        prev_candidate_enabled = getattr(model_to_control, 'enable_candidate', False)
        # 评估时候选开关仅依据固定 N/M 和禁用开关
        enable_eval_candidates = (not getattr(args, 'disable_eval_candidates', False)) and \
            ((getattr(args, 'candidate_top_n', 0) + getattr(args, 'candidate_worst_m', 0)) > 0)
        model_to_control.set_candidate_mode(enable_eval_candidates)
    else:
        enable_eval_candidates = False
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            if args.model_type.find('span')!=-1:
                inputs['mention_pos'] = batch[4]
            # Pass candidate_weight during evaluation (1.0 when enabled)
            if hasattr(model_to_control, 'set_candidate_mode'):
                inputs['candidate_weight'] = 1.0 if enable_eval_candidates else 0.0

            # Optionally override dialogue rounds during evaluation to sync with training
            if hasattr(args, 'eval_dialogue_rounds_override'):
                try:
                    _ovr = int(getattr(args, 'eval_dialogue_rounds_override', -1))
                except Exception:
                    _ovr = -1
                if _ovr >= 0:
                    inputs['dialogue_rounds_override'] = _ovr
    ...
    # 记录评估时是否启用候选增强（固定 N/M 判定）
    results['enable_eval_candidates'] = int(enable_eval_candidates)

    logger.info("Result: %s", json.dumps(results))

    if hasattr(model_to_control, 'set_candidate_mode') and prev_candidate_enabled is not None:
        model_to_control.set_candidate_mode(prev_candidate_enabled)
...
def main():
    ...
    parser.add_argument('--candidate_top_n', type=int, default=2, help="固定 Top-N 候选标签用于增强")
    parser.add_argument('--candidate_worst_m', type=int, default=2, help="固定 Worst-M 候选标签用于增强")
    parser.add_argument('--max_candidate_length', type=int, default=8, help="每对最多使用的候选标签数上限")
                        
    parser.add_argument('--st1_warming_up', type=float, default=0, help='warming-up ratio for basic knowledge learning')
    parser.add_argument('--disable_eval_candidates', action='store_true',
                        help='Disable candidate augmentation during evaluation (default: use same candidate features as training).')

    parser.add_argument('--num_dialogue_rounds', type=int, default=1, 
                        help='Number of dialogue rounds for Relation Negotiator')
    parser.add_argument('--eval_dialogue_rounds_override', type=int, default=-1,
                        help='Override dialogue rounds during evaluation; -1 uses config value')
    ...
    def create_exp_dir(path, scripts_to_save=None):
        ...
        logger.info("Experiment dir: %s", path)
    ...
    # Persist logs to Markdown file for easier inspection after runs
    log_file_path = os.path.join(args.output_dir, "run_log.md")
    os.makedirs(args.output_dir, exist_ok=True)
    if not os.path.exists(log_file_path):
        with open(log_file_path, "w", encoding="utf-8") as lf:
            lf.write("| Time | Level | Logger | Message |\n")
            lf.write("| --- | --- | --- | --- |\n")
    file_handler = logging.FileHandler(log_file_path, encoding="utf-8")
    file_handler.setLevel(logging.INFO if args.local_rank in [-1, 0] else logging.WARN)
    ...
    if data_dir_lower.find('ace')!=-1:
        ...
        if args.no_sym:
            ...
            num_labels = 7 + 7 - 2
    elif data_dir_lower.find('scierc')!=-1:
        num_ner_labels = 7
        ...
        if args.no_sym:
            ...
            num_labels = 8 + 8 - 3
    elif data_dir_lower.find('scinlp')!=-1:
        # SCINLP: 6 entity types (Task, Dataset, Metric, Model, Method, Tool) + NIL = 7
        num_ner_labels = 7

        # label_list length = 12 (包含 NIL 与对称关系头部)
        if args.no_sym:
            # 总类别数 = 2 * 12 - len(sym_labels); 对称仅 NIL
            num_labels = 12 + 12 - 1
        else:
            # 对称关系：compareWith、similarWith，再加 NIL 共 3 个
            num_labels = 12 + 12 - 3
    else:
        assert (False)
    ...
    config.max_seq_length = args.max_seq_length
    config.max_pair_length = args.max_pair_length
    config.max_candidate_length = args.max_candidate_length
    config.candidate_top_n = args.candidate_top_n
    config.candidate_worst_m = args.candidate_worst_m
    # 仅使用固定 N/M 配置，不传递任何自适应标志
    config.num_dialogue_rounds = args.num_dialogue_rounds
    ...
    if args.model_type.startswith('albert'):
        if args.use_typemarker:
            ...
            special_tokens_dict = {'additional_special_tokens': ['[unused' + str(x) + ']' for x in range(4+num_ner_labels+2)]}
    ...
    # Final evaluation when requested
    if args.do_eval:
        logger.info("Running final evaluation...")
        _ = evaluate(args, model, tokenizer, prefix="final", do_test=False)
    ...
    # Collect per-checkpoint records for NDJSON output (one line per point)
    line_records = []
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            eval_result = evaluate(args, model, tokenizer, prefix=global_step, do_test=not args.no_test)
            # Preserve JSON keys as-is for per-line output; one checkpoint per line
            record = dict(eval_result)
            record['checkpoint'] = os.path.basename(checkpoint)
            line_records.append(record)

            # For internal aggregation and best selection, keep suffixed keys
            result = dict((k + '_{}'.format(global_step), v) for k, v in eval_result.items())
            results.update(result)

            # Print one-line summary for this checkpoint
            ckpt_name = os.path.basename(checkpoint)
            logger.info(
                "Checkpoint %s: f1=%.4f f1_with_ner=%.4f ner_f1=%.4f",
                ckpt_name,
                float(eval_result.get('f1', 0.0)),
                float(eval_result.get('f1_with_ner', 0.0)),
                float(eval_result.get('ner_f1', 0.0)),
            )
        ...
        if args.no_test:  # choose best resutls on dev set
            ...
            for k, v in results.items():
                if v > bestv:
                    bestk = k
            logger.info("Best checkpoint key: %s", bestk)

        # Write NDJSON: one checkpoint (point) per line, metric keys unchanged
        output_eval_file = os.path.join(args.output_dir, "results.json")
        # Sort by f1 descending before writing
        sorted_records = sorted(line_records, key=lambda r: r.get('f1', 0.0), reverse=True)
